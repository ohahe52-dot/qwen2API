...
export default function AccountsPage() {
  ...
  // Unlock register when email+password match
  useEffect(() => {
  ...
  const fetchAccounts = () => {
    fetch(`${API_BASE}/api/admin/accounts`, { headers: getAuthHeader() })
      ...
      .catch(() => toast.error(t("accounts.refreshListError")))
  }
  ...
  const handleAdd = () => {
    if (!token.trim()) {
      toast.error(t("accounts.needToken"))
      return
    }
    ...
    fetch(`${API_BASE}/api/admin/accounts`, {
      ...
      .then(data => {
        if (data.ok) {
          ...
          toast.error(localizeError(data.error, t) || t("accounts.injectFailed"), { id, duration: 8000 })
        }
      })
      .catch(() => toast.error(t("accounts.injectRequestFailed"), { id }))
  }

  const handleDelete = (targetEmail: string) => {
    const id = toast.loading(t("accounts.deleting", { email: targetEmail }))
    fetch(`${API_BASE}/api/admin/accounts/${encodeURIComponent(targetEmail)}`, {
      method: "DELETE",
      headers: getAuthHeader(),
    }).then(res => {
      if (!res.ok) throw new Error("delete failed")
      toast.success(t("accounts.deleted", { email: targetEmail }), { id })
      fetchAccounts()
    }).catch(() => toast.error(t("accounts.deleteFailed"), { id }))
  }

  const handleAutoRegister = () => {
    setRegistering(true)
    const id = toast.loading(t("accounts.autoRegistering"))
    fetch(`${API_BASE}/api/admin/accounts/register`, {
      ...
      .then(data => {
        if (data.activation_pending) {
          toast.warning(t("accounts.registeredNeedActivate", { email: data.email }), { id, duration: 8000 })
          fetchAccounts()
        } else if (data.ok) {
          toast.success(data.message || t("accounts.registerSuccess", { email: data.email }), { id, duration: 8000 })
          fetchAccounts()
        } else {
          toast.error(localizeError(data.error, t) || t("accounts.autoRegisterFailed"), { id, duration: 8000 })
  ...
  const handleVerify = (targetEmail: string) => {
    ...
    fetch(`${API_BASE}/api/admin/accounts/${encodeURIComponent(targetEmail)}/verify`, {
      ...
      .then(data => {
        if (data.valid) {
          toast.success(t("accounts.verifySuccess", { email: targetEmail }), { id })
        } else {
          toast.error(t("accounts.verifyFailed", { detail: statusText(data, t) || localizeError(data.error, t) }), { id, duration: 8000 })
        }
        fetchAccounts()
      })
      .catch(() => toast.error(t("accounts.verifyRequestFailed"), { id }))
  ...
  const handleVerifyAll = () => {
    ...
    fetch(`${API_BASE}/api/admin/verify`, {
      ...
      .then(data => {
        if (data.ok) {
          toast.success(t("accounts.verifyAllDone", { concurrency: data.concurrency || 1 }), { id })
        } else {
  ...
  const handleActivate = (targetEmail: string) => {
    ...
    fetch(`${API_BASE}/api/admin/accounts/${encodeURIComponent(targetEmail)}/activate`, {
      ...
      .then(data => {
        if (data.pending) {
          toast.success(t("accounts.activatePending", { email: targetEmail }), { id, duration: 6000 })
        } else if (data.ok) {
          toast.success(data.message || t("accounts.activateSuccess", { email: targetEmail }), { id, duration: 6000 })
        } else {
          toast.error(t("accounts.activateFailed", { detail: localizeError(data.error || data.message, t) }), { id, duration: 8000 })
        }
        fetchAccounts()
      })
      .catch(() => toast.error(t("accounts.activateRequestFailed"), { id }))
  }
  ...
  return (
    <div className="space-y-6 relative">
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
          <Button variant="secondary" onClick={handleVerifyAll} disabled={verifyingAll}>
            <ShieldCheck className={`mr-2 h-4 w-4 ${verifyingAll ? 'animate-pulse' : ''}`} /> {t("accounts.verifyAllBtn")}
          </Button>
          <Button variant="outline" onClick={() => { fetchAccounts(); toast.success(t("accounts.listRefreshed")) }}>
            <RefreshCw className="mr-2 h-4 w-4" /> {t("accounts.refreshBtn")}
          </Button>
          {registerUnlocked && (
            <Button variant="default" onClick={handleAutoRegister} disabled={registering}>
              ...
              {registering ? t("accounts.registeringBtn") : t("accounts.getNewBtn")}
            </Button>
          )}
        </div>
      </div>
      ...
      <div className="grid gap-3 md:grid-cols-5">
        <div className="rounded-xl border bg-card p-4"><div className="text-sm text-muted-foreground">{t("accounts.statValid")}</div><div className="text-2xl font-bold">{stats.valid}</div></div>
        <div className="rounded-xl border bg-card p-4"><div className="text-sm text-muted-foreground">{t("accounts.statPending")}</div><div className="text-2xl font-bold">{stats.pending}</div></div>
        <div className="rounded-xl border bg-card p-4"><div className="text-sm text-muted-foreground">{t("accounts.statRateLimited")}</div><div className="text-2xl font-bold">{stats.rateLimited}</div></div>
        <div className="rounded-xl border bg-card p-4"><div className="text-sm text-muted-foreground">{t("accounts.statBanned")}</div><div className="text-2xl font-bold">{stats.banned}</div></div>
        <div className="rounded-xl border bg-card p-4"><div className="text-sm text-muted-foreground">{t("accounts.statInvalid")}</div><div className="text-2xl font-bold">{stats.invalid}</div></div>
      </div>

      <div className="rounded-2xl border bg-card/40 p-6 space-y-4">
        <div>
          <h3 className="text-base font-bold">{t("accounts.manualInject")}</h3>
          <p className="text-sm text-muted-foreground">{t("accounts.manualInjectHint")}</p>
          <div className="rounded-xl border border-orange-500/30 bg-orange-500/10 p-3 mt-3">
            <p className="text-sm font-semibold text-orange-700 dark:text-orange-300">{t("accounts.manualInjectWarn")}</p>
            <p className="text-xs text-orange-700/80 dark:text-orange-200/80 mt-1">{t("accounts.manualInjectWarn2")}</p>
          </div>
        </div>
        <div className="flex flex-col md:flex-row gap-4 items-end">
          <div className="flex-1 w-full">
            <label className="text-xs font-semibold mb-1.5 block">{t("accounts.tokenLabel")}</label>
            <input type="text" value={token} onChange={e => setToken(e.target.value)} className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm" placeholder={t("accounts.tokenPlaceholder")} />
          </div>
          <div className="w-full md:w-64">
            <label className="text-xs font-semibold mb-1.5 block">{t("accounts.emailLabel")}</label>
            <input type="text" value={email} onChange={e => setEmail(e.target.value)} className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm" placeholder={t("accounts.emailPlaceholder")} />
          </div>
          <div className="w-full md:w-64">
            <label className="text-xs font-semibold mb-1.5 block">{t("accounts.passwordLabel")}</label>
            <input type="text" value={password} onChange={e => setPassword(e.target.value)} className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm" placeholder={t("accounts.passwordPlaceholder")} />
          </div>
          <Button onClick={handleAdd} variant="secondary" className="h-10 w-full md:w-auto font-semibold">
            <Plus className="mr-2 h-4 w-4" /> {t("accounts.injectBtn")}
          </Button>
        </div>
      </div>
      ...
      <div className="rounded-2xl border bg-card/30 overflow-hidden">
        <div className="flex items-center justify-between p-6 border-b bg-muted/10">
          <h3 className="text-xl font-bold">{t("accounts.listTitle")}</h3>
        ...
        <table className="w-full text-sm text-left">
          <thead className="bg-muted/30 border-b text-muted-foreground text-xs uppercase tracking-wider font-semibold">
            <tr>
              <th className="h-12 px-6 align-middle">{t("accounts.colAccount")}</th>
              <th className="h-12 px-6 align-middle">{t("accounts.colStatus")}</th>
              <th className="h-12 px-6 align-middle">{t("accounts.colLoad")}</th>
              <th className="h-12 px-6 align-middle">{t("accounts.colNote")}</th>
              <th className="h-12 px-6 align-middle text-right">{t("accounts.colActions")}</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-border/50">
            {accounts.length === 0 && (
              <tr>
                <td colSpan={5} className="px-6 py-12 text-center text-muted-foreground">{t("accounts.emptyList")}</td>
              </tr>
            )}
            {accounts.map(acc => (
              <tr key={acc.email} className="transition-colors hover:bg-black/5 dark:hover:bg-white/5">
                ...
                <td className="px-6 py-4 align-middle font-mono">
                  <span className="inline-flex items-center justify-center bg-muted/50 px-2 py-1 rounded text-xs border">
                    {t("accounts.threads", { count: acc.inflight || 0 })}
                  </span>
                </td>
                <td className="px-6 py-4 align-middle text-muted-foreground max-w-[420px] truncate" title={statusNote(acc, t)}>
                  {statusNote(acc, t) || "-"}
                </td>
                  ...
                  <div className="flex items-center justify-end gap-2">
                    {acc.status_code !== "valid" && acc.status_code !== "rate_limited" && acc.status_code !== "banned" && (
                      <Button variant="outline" size="sm" onClick={() => handleActivate(acc.email)} className="text-orange-600 dark:text-orange-400 border-orange-500/30 hover:bg-orange-500/10 font-medium">
                        <MailWarning className="h-4 w-4 mr-1" /> {t("accounts.activateBtn")}
                      </Button>
                    )}
                    <Button variant="outline" size="sm" onClick={() => handleVerify(acc.email)} disabled={verifying === acc.email} title={t("accounts.verifyOne")}>
                      {verifying === acc.email ? <RefreshCw className="h-4 w-4 animate-spin text-blue-500" /> : <ShieldCheck className="h-4 w-4" />}
                    </Button>
                    <Button variant="ghost" size="sm" onClick={() => handleDelete(acc.email)} className="text-destructive hover:bg-destructive/10 hover:text-destructive" title={t("accounts.deleteAccount")}>
