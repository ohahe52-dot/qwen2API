  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        toast.error(t("images.generateFailed", { detail: String(detail).slice(0, 80) }))
        return
      }
      ...
      if (newImages.length === 0) {
        setError(t("images.noImageReturned"))
        toast.error(t("images.noImageReturned"))
        return
      }

      setImages(prev => [...newImages, ...prev])
      toast.success(t("images.generateSuccess", { count: newImages.length }))
    } catch (err: any) {
      const msg = err.message || t("images.networkError")
      setError(msg)
      toast.error(t("images.generateFailed", { detail: msg }))
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="space-y-6 max-w-5xl">
      ...
      <div className="rounded-xl border bg-card shadow-sm p-6 space-y-4">
        <div className="space-y-2">
          ...
          />
          <p className="text-xs text-muted-foreground">{t("images.ctrlEnterHint")}</p>
        </div>
        ...
        <div className="flex flex-wrap gap-4 items-end">
          ...
          >
            {loading
              ...
              : <><Wand2 className="h-4 w-4" /> {t("images.generateBtn")}</>
            }
          </Button>
        </div>
        ...
        <div className="rounded-xl border bg-card shadow-sm p-8">
          <div className="flex flex-col items-center justify-center gap-4 text-muted-foreground">
            ...
            <div className="text-center">
              <p className="font-medium">{t("images.loadingTitle")}</p>
              <p className="text-sm text-muted-foreground/70 mt-1">{t("images.loadingSub")}</p>
            </div>
          </div>
        </div>
      )}
      ...
      {images.length > 0 && !loading && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="font-semibold">{t("images.resultTitle", { count: images.length })}</h3>
        ...
        <div className="rounded-xl border bg-card/50 shadow-sm p-12">
          <div className="flex flex-col items-center gap-4 text-muted-foreground">
            ...
            <div className="text-center">
              ...
              <p className="text-sm text-muted-foreground/70 mt-1">{t("images.emptySub")}</p>
            </div>
          </div>
        </div>
      )}
